...
Given("User uploads Dataset1", async ({ page, $test }) => {
  await page.goto("/");
...
const uploadSboms = async (page: Page, files: string[]) => {
  ...
  await expect(
    page.locator("#upload-sbom-tab-content .pf-v5-c-expandable-section__toggle-text")
...
const uploadAdvisories = async (page: Page, files: string[]) => {
  ...
  await expect(
    page.locator("#upload-advisory-tab-content .pf-v5-c-expandable-section__toggle-text")
  ).toContainText(`${files.length} of ${files.length} files uploaded`, {
    timeout: 60_000,
  });
};
