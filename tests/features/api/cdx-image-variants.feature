...
Background:
    ...
    And User has ingested an SBOM that contains a "pedigree/variants" element for a component
...
Scenario Outline: Displaying the variants of a root component
    ...
    Then API should return a response containing an array of variants that have the "VariantOf" relationship with the package
...
Scenario Outline: Displaying the root component of a variant
    ...
    Then API should return a response containing an element in an array of dependencies that has the "VariantOf" relationship with the package
